...
useEffect(() => {
  ...
  // reusable pagination (main bank list)
  ...
  const saveAccountType = async () => {
    if (isSavingAccountType) return; // 🛑 hard guard
    ...
    try {
      ...
      setIsSavingAccountType(false); // ✅ always runs
    }
  };
  ...
  const saveBank = async () => {
    if (isSavingBank) return; // 🛑 prevent double submit
    ...
    try {
      ...
      const payload = {
        ...
        bank_name: bankName, // keep for display
      ...
      await axios.post(`${URL}/banks/add`, payload, {
        headers: { Authorization: token },
      });
  ...
  const saveOpeningBalance = async () => {
    if (isSavingOpeningBalance) return; // 🛑 hard guard
    ...
    try {
      ...
      setIsSavingOpeningBalance(false); // ✅ always runs
    }
  };
