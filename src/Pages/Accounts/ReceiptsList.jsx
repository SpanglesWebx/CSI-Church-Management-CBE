...
export const ReceiptsList = () => {
  ...
  const fetchReceipts = async () => {
    try {
      const res = await axios.get(`${URL}/receipts/list`, {
        headers: { Authorization: token },
        params: {
          page: CurrentPage,
          limit: rowsPerPage,
...
const handleDownload = async () => {
  ...
  try {
    const res = await axios.get(`${URL}/receipts/download-data`, {
      headers: { Authorization: token },
    ...
    if (!data || data.length === 0) {
      showToast("Failed", "No receipts found for selected dates");
      return;
    }
    ...
    autoTable(doc, {
      head: [["Sl No", "Receipt ID", "Date", "Trans No", "Receipt No", "Amount", "Payment", "Bank"]],
      body: tableData,
    });

    doc.save("Receipt_Report.pdf");

    showToast("Success", "Receipt report downloaded");
  } catch (error) {
    console.error("Download error", error);
    showToast("Failed", "Failed to download receipt report");
  }
};
        ...
        <div className="flex items-center justify-between p-2">
          ...
          {["admin", "treasurer"].includes(userRole) && (
            <div className="flex items-center justify-between gap-3">
              <FiDownload size={20} className="text-lavender--600 cursor-pointer" title="Download" onClick={handleDownload} />
...
{["admin", "churchofficeworker"].includes(userRole) && (
          <div className="flex gap-2">
            <button onClick={() => navigate("/admin/receiptslist/Addreceipts")} className="flex items-center gap-2 px-3 py-2 text-white bg-lavender--600 rounded-lg"><FaPlus /> Receipt</button>
          </div>
)} 
        </div>
              ...
              {receipts.length === 0 ? (
                ...
                receipts.map((rec, index) => (
                  <tr key={rec._id} className="border-b">
                    ...
                    <td className="p-2 text-center">
                      {rec.receiptLines?.length > 0 && (
                        <>
                          {rec.receiptLines[0].receiptNumber}

                          {rec.receiptLines.length > 1 && (
                      ...
                      {rec.receiptLines.length > 0 && (
                        <>
                          <span className="text-lavender--600 font-semibold">
                            ({rec.receiptLines[0].ledgerCode})
                          </span>{" "}
                          {rec.receiptLines[0].ledgerName}
                    ...
                    <td className="p-2 text-center">
                      {rec.receiptLines?.length > 0 && (
                        <>
                          {rec.receiptLines[0].isMember
                            ? rec.receiptLines[0].memberName
                            : rec.receiptLines[0].nonMemberName}
                    ...
                    <td className="p-2 text-center">
                      {moment(rec.receiptDate).format("DD-MM-YYYY")}
                    </td>

                    <td className="p-2 text-center flex items-center gap-2">
                      <FaEye size={18}
                        className="cursor-pointer text-lavender--600 mx-auto"
                        onClick={() => navigate(`/admin/receiptslist/ViewReceipts/${rec._id}`)}
                      />
                      {["admin", "treasurer"].includes(userRole) && !rec.realised &&(
                      <BiSolidEditAlt size={20} className="cursor-pointer text-lavender--600 mx-auto" onClick={() => navigate(`/admin/receiptslist/EditReceipts/${rec._id}`)}/>)}
                    </td>
                  </tr>
                ))
              )}
