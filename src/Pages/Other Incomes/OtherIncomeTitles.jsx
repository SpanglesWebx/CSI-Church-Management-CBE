...
export const OtherIncomeTitles = () => {
...
const [jumpInput, setJumpInput] = useState("");
  ...
  const fetchTitles = async () => {
    try {
      ...
      const res = await axios.get(`${URL}/other-income`, {
      ...
      setTitleList(Array.isArray(data) ? data : []);
      setCurrentPage(page || 1);
      setTotalPages(totalPages || 1);
    } catch (err) {
      console.error("Error loading titles:", err?.response?.data || err);
...
useEffect(() => {
  ...
  const saveTitle = async () => {
    ...
    try {
      await axios.post(
        `${URL}/other-income/add`,
        { title: title.trim(), description: titleDesc },
        { headers: { Authorization: token } }
      );
  ...
  const inactivateTitle = async () => {
    if (!selectedTitleId) return;

    try {
      await axios.put(
        `${URL}/other-income/update-status/${selectedTitleId}`,
        {},
        { headers: { Authorization: token } }
      );
  ...
  return (
    <>
      <div className="p-3 mx-1 mt-3 bg-white shadow-md rounded-[10px]">
        <h1 className="text-lg font-semibold">Other Income Titles</h1>
...
/>
        ...
        >
          <p className="text-gray-700 text-center my-4">
            Do you really want to mark this title as{" "}
