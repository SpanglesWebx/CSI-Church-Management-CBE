...
export const YouthAuction = () => {
    ...
    const [youthIdSearch, setYouthIdSearch] = useState("");
  const [youthNameSearch, setYouthNameSearch] = useState("");
  const [youthDropdownById, setYouthDropdownById] = useState([]);
  const [youthDropdownByName, setYouthDropdownByName] = useState([]);
...
// 🔎 Seller (Youth Fellowship) search
  const debouncedSearchYouthById = useRef(
    debounce(async (val) => {
      if (!val) return setYouthDropdownById([]);
      try {
        const res = await axios.get(
          `${URL}/member-search/youth-fellowship/by-id?id=${val}`,
          { headers: { Authorization: token } }
        );
        setYouthDropdownById(res.data || []);
      } catch (err) {
        setYouthDropdownById([
          { member_id: "none", member_name: "No Youth Members found" },
        ]);
      }
    }, 300)
  ).current;

  const debouncedSearchYouthByName = useRef(
    debounce(async (val) => {
      if (!val) return setYouthDropdownByName([]);
      try {
        const res = await axios.get(
          `${URL}/member-search/youth-fellowship?name=${val}`,
          { headers: { Authorization: token } }
        );
        setYouthDropdownByName(res.data || []);
      } catch (err) {
        setYouthDropdownByName([
          { member_id: "none", member_name: "No Youth Members found" },
        ]);
      }
    }, 300)
  ...
  // 📥 Fetch youth auctions
  const fetchAuctions = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${URL}/youth-auctions`, {
        headers: { Authorization: token },
      ...
      console.error("❌ Fetch youth Auction error:", err);
      setAuctions([]);
    } finally {
      setLoading(false);
    }
...
//     try {
//       await axios.post(`${URL}/youth-auctions/add`, formData, {
//         headers: { Authorization: token },
//       });
//       setResponse({ status: "Success", message: "Auction created" });
//       fetchAuctions();
//          // Reset form fields
//     reset();
...
const onSubmit = async (formData) => {
  try {
    ...
    console.error("❌ Youth Auction Error:", err.response?.data || err);

    // ✅ Re-trigger toast for failures
    setResponse({ status: null, message: "" });
    setTimeout(() => {
      setResponse({
        status: "Failed",
    ...
    <>
            <div className="p-3 mx-1 mt-3 bg-white shadow-md rounded-[10px]">
                ...
                <div className="flex items-center justify-between p-4">
                    <h1 className="text-lg font-semibold">Youth’s Auction</h1>
                    <div className="">
                        <label
                            ...
                            {loading ? (
                                ...
                                auctions.map((a, i) => (
                                    <tr key={a._id} className="border-b">
                                        ...
                                        <td className="p-2 text-left">{a.sellerName}</td>
                                        <td className="p-2 text-left">{a.item}</td>
                                        <td className="p-2 text-left">{a.buyerName}</td>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        <div className="p-4 border rounded-lg bg-gray-50">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-lg font-semibold text-gray-700">Seller (Youth Member)</h3>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 relative">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setYouthIdSearch(val);
                                            debouncedSearchYouthById(val);
                                        }}
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm"
                                        value={youthNameSearch}
                                        onChange={(e) => {
                                            const val = e.target.value;
                                            setYouthNameSearch(val);
                                            debouncedSearchYouthByName(val);
                                        }}
                                    />
                                </div>
                                ...
                                {/* ✅ Unified Dropdown */}
                                {(youthDropdownById.length > 0 || youthDropdownByName.length > 0) && (
                                    <ul className="absolute left-1/2 -translate-x-1/2 mt-[65px] w-full bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-40 overflow-y-auto">
                                        {(youthDropdownById.length > 0 ? youthDropdownById : youthDropdownByName).map((m) => (
                                            <li
                                                key={m.member_id}
                                                className="flex px-3 py-2 text-sm text-gray-700 hover:bg-indigo-50 cursor-pointer transition"
                                                onClick={() => {
                                                    setYouthIdSearch(m.member_id);
                                                    setYouthNameSearch(m.member_name);
                ...
                {selectedAuction && (
                    <div className="flex flex-col pt-5 ps-5 w-full max-w-4xl space-y-4">
                        {[
                            ...
                            { label: "Seller", value: selectedAuction.sellerName },
                            { label: "Seller ID", value: selectedAuction.sellerId || "N/A" },
                            { label: "Item", value: selectedAuction.item },
                            { label: "Buyer", value: selectedAuction.buyerName },
                            {
                                label: "Buyer ID",
                                value: selectedAuction.buyerId || selectedAuction.buyerPhone || "N/A"
                            },
