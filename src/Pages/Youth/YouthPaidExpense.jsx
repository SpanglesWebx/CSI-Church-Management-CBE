...
export const YouthPaidExpense = () => {
  ...
  const fetchExpenses = async () => {
    try {
      const res = await axios.get(`${URL}/youthfellow-expenses`, { // Youth Fellowship endpoint
  ...
  const handleCloseBill = async () => {
    ...
    try {
      await axios.patch(
        `${URL}/youthfellow-expenses/close-bill/${selectedExpense._id}`,
        formData,
        { headers: { Authorization: token } }
      );
    ...
    <>
      <div className="p-3 mx-1 mt-3 bg-white shadow-md rounded-[10px]">
        <div className="flex items-center justify-between p-4">
          <h1 className="text-lg font-semibold">Paid Expense - Youth Fellowship</h1>
          {/* Search */}
          <div className="">
