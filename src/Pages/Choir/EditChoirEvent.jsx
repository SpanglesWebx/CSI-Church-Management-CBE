...
export const EditChoirEvent = () => {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `${URL}/choir-members/event/${eventId}`,
          { headers: { Authorization: token } }
        );
        ...
        setCompetitions(
          event.choirCompetitions?.length
            ? event.choirCompetitions
            : [{ competition: "", title: "", participants: [] }]
        );

        const tags = [
          ...new Set(event.choirCompetitions.map(c => c.competition))
        ];
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(
        `${URL}/choir-members/event/update/${eventId}`,
        {
          ...
          eventDate,
          registerBefore,
          choirCompetitions: cleanedCompetitions
        },
        { headers: { Authorization: token } }
      );
      ...
      <div className={`${saving ? "pointer-events-none opacity-60" : ""}`}>
        ...
        <div className="flex justify-start mt-6">
          <FaArrowLeft
            ...
            onClick={() => navigate("/admin/choirevent")}
          />
        </div>
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const res = await axios.post(
                          `${URL}/choir-members/eventBy/save`,
