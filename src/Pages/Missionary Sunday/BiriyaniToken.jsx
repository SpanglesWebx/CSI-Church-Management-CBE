...
export const BiriyaniToken = () => {
  const [isModalOpen, setIsModalOpen] = useState(false); // add income modal
  const [isTokenModalOpen, setIsTokenModalOpen] = useState(false); // price modal
  ...
  const [missionaryDayDate, setMissionaryDayDate] = useState("");
...
const [jumpInput, setJumpInput] = useState("");
  ...
  const fetchIncomeList = async () => {
    try {
      const res = await axios.get(`${URL}/missionary-sunday-biriyani-token-income/list`, {
        headers: { Authorization: token },
        params: {
          page: CurrentPage,
          limit: rowsPerPage,
...
useEffect(() => {
  ...
  const savePrice = async () => {
    ...
    try {
      await axios.post(
        `${URL}/missionary-sunday-biriyani-token-price/add`,
        {
          date: priceDate,
  ...
  const fetchPriceForYear = async (dateStr) => {
    ...
    try {
      const res = await axios.get(`${URL}/missionary-sunday-biriyani-token-price/get-by-year`, {
        headers: { Authorization: token },
        params: { year },
      });

      return res.data.data; // price doc
    } catch (err) {
      // 404 = not found
      return null;
    }
  ...
  // When income date changes: load price for the year and recompute amount if numTokens present
  useEffect(() => {
    const loadAndCompute = async () => {
      ...
      if (!priceDoc) {
        ...
        setAmount("");
        triggerToast("Failed", `Token price not set for year ${new Date(incomeDate).getFullYear()}`);
        return;
      }
      // set price and compute
      const price = Number(priceDoc.price_per_token);
      if (numTokens && /^\d+$/.test(String(numTokens))) {
  ...
  const saveIncome = async () => {
    if (!memberIdSearch || !memberNameSearch || !incomeDate || !missionaryDayDate || numTokens === "") {
    ...
    const payload = {
      ...
      missionary_day_date: missionaryDayDate,
    ...
    try {
      await axios.post(`${URL}/missionary-sunday-biriyani-token-income/add`, payload, {
        headers: { Authorization: token },
      });
  ...
  const openView = async (item) => {
    try {
      if (item && item._id) {
        const res = await axios.get(`${URL}/missionary-sunday-biriyani-token-income/${item._id}`, {
          headers: { Authorization: token },
        });
  ...
  const markAsPaid = async () => {
    ...
    try {
      await axios.put(
        `${URL}/missionary-sunday-biriyani-token-income/mark-paid/${payRecordId}`,
        {},
        { headers: { Authorization: token } }
      );
      ...
      triggerToast("Failed", "Failed to update payment");
    }
  };
  ...
  return (
    <>
      <div className="p-3 mx-1 mt-3 bg-white shadow-md rounded-[10px]">
        <h1 className="text-lg font-semibold">Missionary Sunday Biriyani Token</h1>
        <div className="flex items-center justify-between p-2">
          ...
          <div className="flex gap-2">
            <button onClick={() => setIsTokenModalOpen(true)} className="flex items-center gap-2 px-3 py-2 text-white bg-lavender--600 rounded-lg"><FaPlus /> Token Price</button>
            <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-3 py-2 text-white bg-lavender--600 rounded-lg"><FaPlus /> Token Income</button>
          </div>
        </div>
...
/>
      ...
      {/* SmallModal: Price per token */}
      <SmallModal isOpen={isTokenModalOpen} onClose={() => setIsTokenModalOpen(false)} title="Add Price Per Token">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-2">
          <div>
            <label className="block text-sm font-medium text-gray-700">Date of Token Issued</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Price Per Token</label>
            <input
              type="text"
              value={pricePerToken}
              onChange={(e) => {
                const val = e.target.value;
                if (/^\d*$/.test(val)) setPricePerToken(val);
              }}
      ...
      >
        <p className="text-gray-700 mb-4">
          Are you sure you want to mark this as <b>Paid</b>?
        </p>
      ...
      {/* Modal: Add Token Income */}
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Token Income">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-2">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Missionary Sunday Date</label>
            <input
              type="date"
              value={missionaryDayDate}
              onChange={(e) => setMissionaryDayDate(e.target.value)}
              className="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm"
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700">No. of Tokens</label>
            <input type="text" value={numTokens} onChange={(e) => { const val = e.target.value; if (/^\d*$/.test(val)) setNumTokens(val); }} placeholder="Enter number of tokens" className="block w-full mt-1 border-gray-300 rounded-md shadow-sm sm:text-sm" />
          </div>
      ...
      {/* View modal */}
      <Modal isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Token Income Details">
        {selectedRecord && (
