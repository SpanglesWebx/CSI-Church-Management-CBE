...
export const SundayCoverOffertory = () => {
...
const [jumpInput, setJumpInput] = useState("");
  ...
  // fetch offertory list with pagination, search, date range
  const fetchOffertory = async () => {
    try {
      const res = await axios.get(
        `${URL}/missionary-sunday-cover-offertory/list`,
        {
          headers: { Authorization: token },
          params: {
            page: CurrentPage,
      ...
      // The controller responds with { status, totalPages, offertory }
  ...
  // effect: refetch when dependencies change
useEffect(() => {
  fetchOffertory();
  ...
  const saveOffertory = async () => {
    ...
    if (!memberIdSearch || !memberNameSearch || !amount || !date) {
      // small toast pattern to retrigger
    ...
    try {
      await axios.post(
        `${URL}/missionary-sunday-cover-offertory/add`,
        {
      ...
      // success toast
      setResponse({ status: null, message: "" });
      setTimeout(() => setResponse({ status: "Success", message: "Cover Offertory added successfully" }), 10);
      setTimeout(() => setResponse({ status: null, message: "" }), 3000);
  ...
  // view single record (fetch fresh data optional)
  const openView = async (item) => {
    // you already have item, but to be safe we can fetch single endpoint if needed
    try {
      if (item && item._id) {
        const res = await axios.get(`${URL}/missionary-sunday-cover-offertory/${item._id}`, {
          headers: { Authorization: token },
        });
  ...
  return (
    <>
      <div className="p-3 mx-1 mt-3 bg-white shadow-md rounded-[10px]">
        <h1 className="text-lg font-semibold">Missionary Sunday Cover Offertory</h1>
        <div className="flex items-center justify-between p-2">
          ...
          <button onClick={openAddModal} className="flex items-center gap-2 px-5 py-2 text-white bg-lavender--600 rounded-lg">
            <FaPlus /> Cover Offertory
          </button>
        </div>
...
/>
      ...
      {/* Add Modal */}
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Cover Offertory">
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button onClick={saveOffertory} className="px-4 py-2 bg-lavender--600 text-white rounded-md">Save</button>
        </div>
      </Modal>
