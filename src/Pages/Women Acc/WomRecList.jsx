...
export const WomRecList = () => {
  ...
  const fetchReceipts = async () => {
    try {
      const res = await axios.get(`${URL}/women-receipts/list`, {
        headers: { Authorization: token },
        params: {
          page: CurrentPage,
          limit: rowsPerPage,
  ...
  const handleDownload = async () => {
    ...
    try {
      const res = await axios.get(`${URL}/women-receipts/download-data`, {
        headers: { Authorization: token },
      ...
      autoTable(doc, {
        head: [["Sl No", "Date", "Trans No", "Receipt No", "Amount", "Payment", "Bank"]],
        body: tableData,
      });

      doc.save("Women_Receipt_Report.pdf");
    ...
    <>
      <div className="p-3 mx-1 mt-3 bg-white shadow-md rounded-[10px]">
        <div className="flex items-center justify-between p-2">
          <h1 className="text-lg font-semibold">Women Receipts</h1>
        ...
        <div className="flex items-center justify-between p-2">
          ...
          {["admin", "churchofficeworker"].includes(userRole) && (
            <div className="flex gap-2">
              <button
                onClick={() => navigate("/admin/womenReceipt/womenAddRec")}
                  ...
                  <tr key={rec._id} className="border-b">
                    ...
                    <td className="p-2 text-center flex items-center gap-2">
                      ...
                      <FaEye
                        ...
                        onClick={() => navigate(`/admin/womenReceipt/womenViewRec/${rec._id}`)}
                      />
                      ...
                      {["admin", "treasurer"].includes(userRole) && !rec.realised &&(
                        <BiSolidEditAlt
                          size={20}
                          className="cursor-pointer text-lavender--600 mx-auto"
                          onClick={() => navigate(`/admin/womenReceipt/womenEditRec/${rec._id}`)}
                        />
                      )}
