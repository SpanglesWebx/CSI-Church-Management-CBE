...
export const WomJournal = () => {
  ...
  useEffect(() => {
    if (activeCreditorIndex >= 0 && creditorItemRefs.current[activeCreditorIndex]) {
      creditorItemRefs.current[activeCreditorIndex].scrollIntoView({
        block: "nearest",
        behavior: "smooth"
      });
    }
  ...
  const fetchJournals = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${URL}/women-journals/list`, {
        headers: { Authorization: token },
        params
      });
  ...
  const handleAccountSelect = (item) => {
    ...
    else if (activeTarget.scope === "row") {
  ...
  const handleEdit = async (id) => {
    ...
    try {
      ...
      const res = await axios.get(`${URL}/women-journals/${id}`, {
        headers: { Authorization: token }
      });
  ...
  const handleSave = async () => {
    ...
    if (!journalDate)
      return showToast("Failed", "Please select date");

    if (!headerAmount || Number(headerAmount) <= 0)
      return showToast("Failed", "Enter header amount");

    if (!isAmountMatched)
      return showToast("Failed", "Amount mismatch");
    ...
    const payloadEntries = entries.map((r) => {
      ...
      const ledgerSource =
        isDebitHeader ? r.creditLedger || {} : r.debitLedger || {};

      const ledgerNameFallback =
        isDebitHeader ? r.creditAccount : r.debitAccount;

      return {
        type,
        ledger: {
          key: ledgerSource.key || "",
          ledgerName: ledgerSource.ledgerName || ledgerNameFallback || "",
    ...
    const payload = {
      ...
      headerLedger: headerLedger
        ? {
          key: headerLedger.key,
          ledgerName: headerLedger.ledgerName,
    ...
    try {
      ...
      if (isEditMode) {
        ...
        res = await axios.put(
          `${URL}/women-journals/update/${editId}`,
          payload,
          { headers: { Authorization: token } }
        );
  ...
  return (
    <>
        ...
        <div className="overflow-x-auto mt-4">
          <table className="w-full text-sm text-gray-500">
            ...
            <tbody>
              {journals.length === 0 ? (
                <tr>
                  <td colSpan="8" className="p-3 text-center text-gray-500">No journal entries found</td>
                </tr>
              ) : (
      ...
      {/* ---------------- Journal Modal ---------------- */}
      <JournalModal isOpen={isModalOpen} onClose={() => {
        setIsModalOpen(false);
        setIsEditMode(false);
        setEditId(null);
        ...
        <div className="flex justify-end mt-4">
          <button onClick={handleSave} disabled={!isAmountMatched} className={`px-4 py-2 rounded text-white ${isAmountMatched ? "bg-lavender--600" : "bg-gray-300 cursor-not-allowed"}`}>
