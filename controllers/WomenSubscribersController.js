...
exports.getWomenSubscribers = async (req, res) => {
  try {
    ...
    const subscribersAgg = await Subscription.aggregate([
      // ✅ Only last 3 financial years
      { $match: { year: { $in: last3Years } } },

      // ✅ Calculate total monthlySubscriptionOffering
      {
        $addFields: {
          yearTotal: {
            $add: [
    ...
    const totalCount =
      subscribersAgg[0].totalCount.length > 0
        ? subscribersAgg[0].totalCount[0].count
        : 0;
    ...
    console.error("Women Subscribers error:", err);
    res.status(500).json({
      message: "Failed to fetch women subscribers",
      error: err.message,
    });
  }
};
