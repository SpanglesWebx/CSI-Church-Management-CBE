...
exports.addEvent = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(eventBy)) {
      return res.status(400).json({
        status: "Failed",
        message: "Invalid Event By selected"
      });
    }
    ...
    if (registerDateObj > eventDateObj) {
      return res.status(400).json({
        status: "Failed",
        message: "Register before date must be before Event date"
      });
    }

    /* ===============================
       CLEAN STUDENT COMPETITIONS
    =============================== */

    studentCompetitions = studentCompetitions
      .map(c => c.trim())
      .filter(Boolean);

    teacherCompetitions = teacherCompetitions
      .map(c => c.trim())
      .filter(Boolean);
...
exports.getEventById = async (req, res) => {
  try {
    ...
    const event = await SundaySchoolEvent
...
exports.updateEvent = async (req, res) => {
  try {
    ...
    if (classEvents.length === 0 && teacherCompEvents.length === 0) {
      return res.status(400).json({
        status: "Failed",
        message: "At least one competition must exist"
      });
    }
...
exports.getSundaySchoolEventById = async (req, res) => {
  try {
    const { id } = req.params;
