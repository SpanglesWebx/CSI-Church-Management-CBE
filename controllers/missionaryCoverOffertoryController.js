const Offertory = require("../Schema/MissionarySundayCoverOffertory");
...
exports.addOffertory = async (req, res) => {
  try {
    ...
    res.json({ status: "Success", message: "Cover Offertory added", data: createData });
  } catch (err) {
    console.error("❌ Error adding cover offertory:", err);
...
exports.listOffertory = async (req, res) => {
  try {
    ...
    console.error("❌ Error fetching offertory list:", err);
    res.status(500).json({ status: "Failed", message: "Server error" });
  }
};
