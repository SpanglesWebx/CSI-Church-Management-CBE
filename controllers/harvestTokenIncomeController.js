const TokenIncome = require("../Schema/HarvestBiriyaniTokenIncome");
...
if (!member_id || !member_name || !date_of_issue || !harvest_day_date || num_tokens === undefined) {
...
const year = new Date(date_of_issue).getFullYear();  // use issue-date
    ...
    if (!priceDoc) {
      return res.status(400).json({
        status: "Failed",
        message: `Token price not set for year ${year}`,
      });
    }
  ...
  payment_status: "Unpaid",
    ...
    return res.json({
      status: "Success",
      message: "Token income saved",
      data: doc,
    });
  } catch (err) {
    console.error("❌ Harvest addIncome error:", err);
...
exports.listIncomes = async (req, res) => {
  try {
    ...
    const incomes = await TokenIncome.find(filter)
      .sort({ date: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("❌ Harvest listIncomes error:", err);
    return res.status(500).json({
      status: "Failed",
      message: "Server error",
    });
...
exports.getIncomeById = async (req, res) => {
  try {
    ...
    console.error("❌ Harvest getIncomeById error:", err);
    return res.status(500).json({
      status: "Failed",
      message: "Server error",
    });
